    ...
    return(
        <div className="h-screen overflow-y-hidden">
            ...
            <div className="w-full  h-3/5 flex items-center justify-center flex-col gap-y-2">
                <Enter className="btn" autoFocus handleEnter = {handleEnter}/>
                ...
                enter && (
                    <motion.div
                         ...
                         <img className="w-20 -translate-y-10"  src={connect}></img>    
                            ...
                            <div className="text-center flex-col gap-y-0.5">
                                <p className="text-xs text-gray-400">Before entering</p>
                                <p className=" text-gray-50">Give yourself a cool Username</p>
                            </div>
                            <form className="max-w-xs w-9/12 flex items-center justify-evenly flex-col gap-y-4 md:w-1/4">
                                    <input autoFocus autoComplete="name" className="rounded border-0 outline-0 py-4 px-3 text-base w-full md:py-1.5 text-xs" type="text" placeholder="John Doe"/>
                                    <button type="submit"onSubmit={()=> handleEnter() } className="sm:rounded btn py-4 bg-white text-base text-white border-0 outline-0 w-full md:py-2 text-xs">ENTER</button>
                            </form>
                            <p className="cursor-pointer font-light text-gray-300 text-xs underline" onClick={()=> handleEnter()}>Go back</p>
                        </div>
                    </motion.div>
                )
            }
